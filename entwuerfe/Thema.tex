...
übermitteln, welche die Feeds ihrerseits an Nutzer (Subscriber) weiterleiten, die bei jenen registriert sind. Dies würde jedoch bedeuten,
dass sowohl auf Nutzer- als auch auf RSS-Server-Seite die schon bestehenden Anwendungen durch neue ersetzt werden müssten.
Solch ein Ansatz würde sicherlich auf
Akzeptanzschwierigkeiten stoßen. Um dies zu verhindern, bietet sich folgendes Konzept an: Die Rolle der RSS-Server bleibt bestehen und ändert sich nicht.
...
bereits in dem Publish/Subscribe-System REBECA \cite{MuFiBu:2001:ArchFrameECommApp} umgesetzt.
...
Das Overlay-Netzwerk sollte dabei folgende Eigenschaften erfüllen:
\begin{itemize}
...
\item Dem Netz spontan beigetretene Broker sollen erforderliche Parameter automatisch beziehen bzw. einstellen:
  ...
  Unabhängig vom momentanen Zustand soll das Netz fähig sein, nach gewisser Zeit in einem gültigen Zustand zu landen
  (z.B. korrekte Information jedes Brokers über verbundene Subscriber oder Subnetzgrößen):
  $\longrightarrow$ Selbststabilisierung\\
  Das Netz soll vorübergehende Fehler in der Netzkonfiguration überstehen und selbständig beseitigen (z.B eventuelle Zyklen im
  Netz):
  $\longrightarrow$ Selbstheilung\\
