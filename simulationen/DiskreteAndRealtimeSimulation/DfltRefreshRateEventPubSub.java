...
public class DfltRefreshRateEventPubSub extends EventPubSub {
	...
	protected long calculateNextRequestTimeout() {
		...
		// diff)) * 1000;
		return (long) ((new Random().nextFloat() * (getPreferredPollingRate()) + (ttl - diffsecs)) * 1000);
	}
	...
	protected synchronized void updateRequestTimerByOldFeed() {
		updateRequestTimer(getPreferredPollingRate() * 1000);
	}
	...
	synchronized protected void updateRequestTimer(long interval) {
		...
		feedRequestTimer.schedule(feedRequestTask, interval, getPreferredPollingRate() * 1000);
	...
	protected class InfoWindowExtension implements ChangeListener {
		...
		public void stateChanged(ChangeEvent e) {
			...
			if ( slider.getValueIsAdjusting() == false ) {
				((PubSubNode) baseWindow.getNode()).setPreferredPollingRate(slider.getValue());
			}
