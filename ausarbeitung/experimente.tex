...
Die tatsächlichen Nachrichtenlaufzeiten berechnen sich aus $engineTimerPeriod\cdot rss\-Feed\-Msg\-RT$ bzw. $engineTimerPeriod\cdot rss\-Feed\-Request\-Msg\-RT$.
...
Die Zeitdauer bei jeder Messung betrug 6.000 Ticks bzw. simulierte Sekunden.
...
Vorteile das vorgestellte Verfahren gegenüber dem herkömmlichen Verfahren bietet.\\
...
(Gütefunktion $\Gamma_A$) in Ab\-hän\-gig\-keit von der Anzahl der Klienten im Netzwerk. Ausgangsparameter für dieses Experiment sind eine Netzgröße von 2001 Knoten,
...
Die Zeiten entsprechen eher realen Bedingungen. So kann ein Schwellenwert der Anzahl der Klienten bestimmt werden, bei dem eine Divergenz der Dienstgüte eintritt.
Der Zeitpunkt, zudem die  Messung der Dienstgüte stattfand, lag bei 6.000 Ticks. In der Abbildung ist zu sehen, dass
bereits bei einer Anzahl von 24 Klienten eine Verbesserung des Aktualitätsgrades der Nachrichten eintritt. Mit zunehmender Zahl der Klienten im Netzwerk steigt auch
der Grad der Verbesserung der Dienstgüte.
...
auf die Streuung wird hierbei unterbunden. Damit sind die Variationskoeffizienten bei unterschiedlichen Messreihen mit verschiedenen Mittelwerten trotzdem vergleichbar.
Es zeigt sich, dass der Variationskoeffizient nach einer gewissen Zeit relativ stabil bleibt und mit der Serverbelastung korreliert.\\
...
In Anlehnung an das vorige Experiment (keine Staukontrolle) untersuchen wir hier die Adaptionsgüte für den Ansatz, dass sich nur ein Publisher im System befindet. Es ist irrelevant,
...
In diesem Experiment übernimmt ein Klient die Rolle des Publishers und pollt den RSS-Server mit einer Polling-Periode von 1 Sekunde. Abbildung
...
Abbildung \ref{Abb:Ein_Publisher_Nachrichtenverzoegerung} zeigt die Nachrichtenverzögerung $\Gamma_V$. Diese ist deutlich höher als beim Referenzexperiment. Auch wenn sich das Experiment
auf ein spezielles Szenario bezieht, so macht es deutlich, dass bei ungünstigen Netzwerken (große Netzwerke, geringer Grad der Verzweigung zwischen Brokern, lange Übertragungszeiten)
der in dieser Arbeit vorgestellte Ansatz dem Ansatz mit nur einem Publisher vorzuziehen ist.
...
Churn beginnt beim Zeitpunkt 3000 und endet beim Zeitpunkt 5000. Dabei sind nach 900 Ticks 80\% der Klienten ausgetauscht.
...
dargestellt, die innerhalb einer festen Zeit (900 Ticks) das System dynamisch betreten und verlassen. Abbildung
...
\importsmallgnuplotps{Serverbelastung: Nachrichtenlaufzeit = 1 Tick}{Abb:Serverbelastung_NLZ_1_sekunde}
...
\importsmallgnuplotps{$\Gamma_A$: Nachrichtenlaufzeit = 1 Tick}{Abb:Gamma_A_NLZ_1_sekunde}
...
eine verminderte Leistungsfähihkeit bei anderen auf dem Weg zwischen Sender und Empfänger liegenden Knoten zu simulieren. In diesem Experiment wollen
...
die Adaptionsgüte $\Gamma_A$.} im Vergleich zum Referenzgraphen auswirken. Im Vergleich zum Referenzgraphen (siehe Abbildung \ref{Abb:Referenzverlauf_small})
ist zu erkennen (siehe Abbildung \ref{Abb:Serverbelastung_NLZ_1_sekunde}), dass zu Beginn der Simulation geringfügig mehr Zeit bis zum Erreichen des maximalen
Füllgrades der Serverqueue benötigt wird. Dies bedeutet, dass der Server in dieser Zeitspanne einer geringeren Belastung unterliegt. Die Mehrbelastung im
...
den Subscriber erreicht. Auch innerhalb der Joinphase unterliegt der Server einer höheren Belastung im Referenzgraphen. Durch die langen Nachrichtenlaufzeiten hat die
Churn-Kompensation einen geringeren Einfluss auf die Anpassung der Pollingperioden als im Graphen \ref{Abb:Serverbelastung_NLZ_1_sekunde}, da Antworten der entsprechenden
Broker einen Subscriber bisweilen zu spät erreichen. Im weiteren Verlauf der Graphen ist zu erkennen, dass der Graph in Abbildung \ref{Abb:Serverbelastung_NLZ_1_sekunde}
dichter an der Queuegröße liegt, als der Graph des Referenzverlaufs. Bei kurzen Nachrichtenlaufzeiten unterliegt der Server hier also einer geringeren Belastung, als
bei längeren Nachrichtenlaufzeiten. Abbildung \ref{Abb:Gamma_A_NLZ_1_sekunde} zeigt bei kurzer Nachrichtenlaufzeit den Verlauf der Adaptionsgüte $\Gamma_A$,
welche sich schnell bei 100\% einpegelt.\\
Um diese Arbeit in angemessenem Rahmen zu halten, haben wir auf weitere Analysen bezüglich Nachrichtenlaufzeiten verzichtet.
