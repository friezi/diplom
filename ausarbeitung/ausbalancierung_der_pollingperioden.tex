...
Aber was soll passieren, wenn ein Subscriber einen Feed über das Notifikationssystem erhält, während schon mehrfache Retransmissionen auftraten und der Subscriber
sich gerade in der Messung der Roundtrip-Time befindet ($RT$ läuft also)? Die Messung und die Retransmissionen sollten weiter laufen. Nach dem bisherigen
Konzept dient als
...
Der so gemessene $rtt$ steht mit Sicherheit in
keiner Beziehung zur eigentlichen Roundtrip-Time. Um diesem Phänomen vorzubeugen, müssen wir einen neuen Wert definieren, den $initialcpp$, welcher als
Berechnungsgrundlage für $rto$ und $rtt$ dienen soll und den $cpp$ in diesem Zusammenhang ersetzt. Bei jedem Setzen des $RQT$ wird $initialcpp$ auf
$initialcpp:=cpp$ gesetzt. Während einer Messung der Roundtrip-Time kann nun der $cpp$ modifiziert werden, ohne dass dies einen Einfluss auf den $rto$ hat.
...
ermitteln, oder die Abtastrate (sprich die Rate der Feed-Requests) war höher. In diesem Fall setzen wir $initialcpp:=cpp$ (da der $cpp$ schon aufgrund des
...
Unsere Algorithmen müssen wir also wie folgt modifizieren:
