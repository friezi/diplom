...
Zeit verschiedene Internet-Dienste entwickelt worden. So können Informationen als E-Mail (elektronische Post) versendet werden.
Sollen Informationen einer breiten Öffentlichkeit zugänglich gemacht werden, so können diese auf einer Webseite präsentiert werden.
Für statische Informationen, die keinem stetigen Wechsel unterliegen, ist dies ein geeignetes Verfahren. In zunehmendem Maße weisen im Internet präsentierte Informationen
einen stark dynamischen Charakter auf. Daher hat sich in neuerer Zeit u. a. die Benutzung von Wikis stark verbreitet.
Ein Wiki ist eine Sammlung von WWW-Seiten zu einem bestimmten Thema, die von Benutzern sowohl gelesen als auch online verändert werden kann. Es bietet die Möglichkeit,
neue Informationen schnell im WWW zu veröffentlichen.
Bei dieser Art der Präsentation von Informationen spricht man von einem pull-basierten Ansatz, da ein Interessent den Informationsanbieter (bzw. seine Webseite) aktiv
kontaktieren muss, um einen aktuellen Stand der Informationen zu erhalten. 
Ist der Informationsanbieter der Initiator der Informationsübermittlung (Bsp. E-Mail), so spricht man von einem push-basierten Ansatz der Informationsverteilung.
Ein allgemeines Kommunikationsmodell, um Interessenten über inhaltliche Änderungen von im Internet präsentierten Informationen zu unterrichten,
ist Publish/Subscribe. Es kann sowohl dem push- als auch dem pull-basierten Ansatz folgen, obwohl in diesem Zusammenhang der push-basierte Ansatz häufiger anzutreffen ist.

\section{Motivation}
Im Kommunikationsmodell von verteiltem Publish/Subscribe (kurz: Pub/Sub) finden sich drei
...
Aufgrund der indirekten Verbindung zwischen Subscribern und Publishern über die Broker brauchen Publisher nicht eine Vielzahl von
