...
\texttt{SimParameters} definiert werden können. Ein RSSServer generiert nur ganz
...
\texttt{newFeed()} eine Event-Feed generiert, so wird genau ein neues Event erzeugt, welches in den bisherigen Feed Eingang findet. Das älteste Event wird dabei entfernt.\\
...
RSSFeed der Klasse \texttt{RSSFeedMessage}. Der empfangende Subscriber (Publisher) erzeugt einen neuen RSS-Feed mit ausschließlich den für ihn neuen Events und sendet diesen an den
...
Mit dem Parameter \texttt{actionFile} kann eine Datei definiert werden, in der zeitabhängige Aktionen festgesetzt werden können. In der ersten Spalte
