...
Dabei soll erreicht werden, dass die maximal mögliche Bandbreite (begrenzt durch die minimale Bandbreite (\glqq bottleneck\grqq{}) auf dem Verbindungsweg, s. o.) voll
...
ist \glqq self-clocking\grqq{}: da $acks$ erst nach Erhalt der entsprechenden Datenpakete versendet werden können, bestimmt die Rate der ankommenden $acks$ die
...
hier zunächst eine feste Rate gewählt werden muss. Diese wird zu Beginn relativ niedrig gewählt, bzw. ein Staufenster \glqq congestion window\grqq{} ($cngw$) 
...
Für jedes $ack$ wird das Staufenster um den Betrag 1 erhöht. Begrenzt wird dessen Größe durch das \glqq advertised receiver window\grqq{},
...
obere Schranke für den $rto$, $srtt$ ist die \glqq smoothed roundtrip time\grqq{} und
wird wie folgt ermittelt:
\begin{equation}
  srtt= \alpha \cdot  srtt+ (1 - \alpha) \cdot  rtt
\end{equation} 
$\alpha$ ist ein ebenfalls empirisch ermittelter Glättungsfaktor (\glqq smoothing factor\grqq{}). Empfohlene Werte sind für $\alpha:$ $0.8 \sim 0.9$ und für $\beta:$
...
Dieser Algorithmus trägt zur Stauvermeidung (\glqq congestion avoidance\grqq{}) bei und besteht parallel zum \glqq slow-start\grqq{}-Algorithmus. Van Jacobson gibt in
...
Grundsätzlich wird der $rto$ nach einem Timeout vergrößert (\glqq back-off\grqq{}).
...
ermittelte (\glqq backed-off\grqq{}) $rto$ als Grundlage für die nächste Wiederholung bzw. für die Aussendung des nächsten Datenpaketes herangezogen. Nur wenn ein $ack$
...
Datenpaketes das Netz nicht wesentlich im Negativen beeinflusst. \glqq Fast retransmit\grqq{} sollte, muss aber nicht, von einer konkreten TCP-Implementation unterstützt
werden.\\
