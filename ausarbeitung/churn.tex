...
Die Auswirkungen von Churn können verschiedenartig sein. So kann Churn beispielsweise bei BitTorrent \cite{BitTorrent} dazu führen, dass sich Downloadzeiten
verlängern, falls Klienten das System verlassen, oder dass bestimmte Dateien nicht zugreifbar sind, falls ein Tracker ausfällt. Bei DHT-basierten Netzen
(neuere BitTorrent-Versionen sind mittlerweile DHT-basiert) kann schon der
vorüber\-gehen\-de Verlust eines Nachbarknotens zu Performanzeinbußen führen (Effizienz ist ein Designziel bei DHT-basierten Peer-To-Peer-Systemen),
da der Ausgangsknoten gezwungen sein kann, suboptimale Routen zu wählen \cite{Rhea2004}.
...
können \cite{Rhea2004}. Stutzbach und Rejaie \cite{Stutzbach2004} haben eine Reihe von Techniken entwickelt und in einem von ihnen entwickelten Tool
...
geographisch verteilten Peers. Darüber hinaus wird jeder Client-Prozess direkt durch einen Benutzer gesteuert. Das heißt, das \glqq Verhalten der User-Clients
repräsentiert vollständig Benutzer-gesteuerte Aspekte dynamischer Mitgliedschaften\grqq{} im System \cite{Stutzbach2004}. Ergebnisse einer solchen
Untersuchung können
damit für Peer-To-Peer-Systeme auf gleicher Basis herangezogen werden. Nach Stutzbach und Rejaie entspricht die Verteilung der Dauern, wie lange sich
...
halten bzw. das Netzwerk schnell in einen solchen wieder zurückzuführen. Da diese Techniken keine Relevanz in bezug auf Churn bei \pubsubrss haben,
wollen wir hier nicht näher darauf eingehen und verweisen auf die in der Literaturliste angegebenen Fachartikel.
...
können zwischen den Teilbäumen (\glqq Inseln\grqq{}) keine Nachrichten mehr
...
desto geringer wird folglich der Aktualitätsgrad der RSS-Feeds in Inseln sein, in denen ein guter Aktualitätsgrad durch den Erhalt
von RSS-Feeds über das Notifikationssystem zustande kam. Eine Anpassung der
Polling-Perioden der Subscriber an eine veränderte Serverbelastung wird einen längeren Zeitraum beanspruchen, da weniger RSS-Feeds über das
Notifikationssystem verteilt werden und somit eine Ausbalancierung der Polling-Perioden in gerimgerem Maße erfolgen wird.\\
...
(Supernodes bei Gnutella und KaZaA \cite{Aberer:P2P,KaZaA}). Daher tritt Churn auf Brokerebene in Abhängigkeit des jeweiligen Notifikationssystems auf.
Um die Auswirkungen
...
Verlassen Klienten das System, so hat das weniger Feed-Requests pro Zeiteinheit zur Folge, woraufhin die übrigen
...
vermieden werden. Eine Lösung kann aufgrund folgender Überlegung entwickelt werden: tritt ein Subscriber dem System bei, so existieren eventuell schon
genügend Subscriber, welche einen
...
Erhält ein Subscriber eine einen RSS-Feed vom Typ \texttt{InitialBrokerRSSFeed}, so muss er pro enthaltenem RSS-Server, der von ihm verwaltet wird, die Operation
