...
aussagekräftigen $artt$ und $cpp$ berechnet haben. der neu hinzugekommene Subscriber kann sich also an diesen Richtwerten orientieren. Es wäre mit einigem Aufwand
...
\ref{cs:ausbalancierung_der_polling-perioden}), kann dieser den enthaltenen Wert des $feed.artt$ zwischenspeichern. Betritt nun ein neuer Subscriber das System, so kann
der Broker diesem den aktuellen Feed mitsamt des zwischengespeicherten $artt$-Wertes übermitteln. Der neue Subscriber wählt nun wie bisher seinen $cpp$ auf Basis dieses
$feed.artt$-Wertes. Dadurch steigen neue Subscriber bei der Messung der Roundtrip-Time bereits mit einer höheren $cpp$ ein, wovon wir uns eine Entlastung des RSS-Servers
