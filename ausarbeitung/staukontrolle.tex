...
mitbestimmend für den $RT$ der $rto$. Aufgrund der vorherigen Definition müssen wir sagen $rto:=cpp$. Wir werden im Weiteren nur noch mit $cpp$ arbeiten, der
ebenfalls je nach Situation unterschiedlich berechnet wird. Um eine klarere Differenzierung vorzunehmen, werden wir trotzdem zwischen den Begriffen 
$RQT$ und $RT$ unterscheiden. In der Implementierung der Simulationsumgebung wurden jedoch ausschließlich die Begriffe $RQT$ und $cpp$ verwendet. Grundlage
für das Aufsetzen der Timer $RQT$ und $RT$ bildet also der $cpp$.\\
...
Wir wollen ebenfalls eine obere Schranke für $cpp$, den $mpp$ (steht für maximale Polling-Periode), definieren. Dieser vermeidet, dass bei ungünstigen
...
Neuberechnungen von $cpp$ unhd $rtt$ finden zunächst nur dann statt, wenn ein Subscriber einen RSS-Feed von einem RSS-Server erhält.
Weitere Verbesserungen werden wir in Abschnitt \ref{cs:ausbalancierung_der_polling-perioden} betrachten.\\

$rtt$ kann anscheinend eindeutig bestimmt werden (warum nur anscheinend, werden wir später erörtern), bevor der $RT$ das erste Mal abgelaufen ist. Ist der $RT$
bereits zum wiederholten Male abgelaufen, und bekommt ein Subscriber einen RSS-Feed, so haben wir hier das gleiche Problem, welches uns schon von TCP her als
``retransmission ambiguity'' (siehe \ref{csp:karns_algorithmus}) bekannt ist: wir können nicht bestimmen, durch welchen Feed-Request der ankommende Feed ausgelöst
wurde. $rtt$ lässt sich also nicht eindeutig bestimmen. 

\paragraph{Berechnung von $cpp$}
\begin{description}
  \item [Fall I: Subscriber erhält RSS-Feed vom RSS-Server:]
    Grundsätzlich dient der Wert $rtt$ als Bezugspunkt für $cpp$. Erhält ein Subscriber einen RSS-Feed von einem RSS-Server, kann $rtt$ berechnet werden.
    $cpp$ berechnet sich dann wie folgt: \[cpp:=min\{mpp,max\{rtt,ppp\}\}\]
  ...
  \item [Fall II: $RT$ läuft ab:]
    In diesem Fall wird $cpp$ einfach verdoppelt, also \[cpp:=2*cpp\]
    Dies bedeutet exponentielles Wachstum von $cpp$. Damit das System skalierbar ist und auch bei großen Netzen funktioniert, muss die Anpassung exponentiell
    erfolgen (siehe ``exponential retransmit timer backoff'' in Abschnitt \ref{css:tcp}).
\end{description}
\paragraph{Berechnung von $rtt$:}
