...
mitbestimmend für den $RT$ der $rto$. Aufgrund der vorherigen Definition müssen wir sagen $rto:=cpp$. $cpp$ wird ebenfalls je nach Situation unterschiedlich
berechnet. Um eine klarere Differenzierung vorzunehmen, werden wir trotzdem dort zwischen den Begriffen 
$RQT$ und $RT$ bzw. $cpp$ und $rto$ unterscheiden, wo es der Deutlichkeit halber angebracht erscheint.
In der Implementierung der Simulationsumgebung wurden jedoch ausschließlich die Begriffe $RQT$ und $cpp$ verwendet.\\
...
Stelle eine mittleren Wert berechnen, um die Differenz zur eigentlichen Roundtrip-Time möglichst gering zu halten. Hat es $i$ Retransmissionen gegeben, so gilt
$rto=2^i*cpp$. Dann wählen wir $rtt:=2^{i/2}*cpp$. Dies ist zwar nicht die mittlere Roundtrip-Time, aber die Roundtrip-Time bezogen auf die
mittlere Retransmission. Der Ansatz, die Berechnungen von $cpp$ und $rto$ zu trennen, ist dem von Karn sehr ähnlich, nur ist die Berechnung eine andere. Hier kann
...
Wie eingangs beschrieben gibt es nur eine konzeptionelle Unterscheidung zwischen $cpp$ und $rto$. In der Praxis brauchen wir nur $cpp$ zu verwenden, welcher als
Stellwert für den $RT$ dient.
...
Szenario: aufgrund abgelaufener Retransmissiontimer hat ein Subscriber einige Feed-Requests mehrfach ausgesandt. Diese sind aber nicht verloren gegangen, sondern
...
verbesserte Server-Reaktionsfähigkeit erwünscht ist und andererseits nicht alle Subscriber gleichzeitig den $rtt$ auf diese Weise ermitteln werden. Dies ist jedoch
eine Wunschvorstellung, von der man nicht ausgehen kann. Tatsächlich kann es passieren, dass es bei einer plötzlichen Erreichbarkeit des Servers zu einem Ansturm
auf diesen kommt. Wir werden aber
in Zusammenhang mit der vorgestellten Technik in Abschnitt \ref{cs:ausbalancierung_der_polling-perioden} diese Phänomen eindämmen können. 
