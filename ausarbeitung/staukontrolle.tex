...
die Sequenznummer \cite{RFC2581}. Anhand der Sequenznummer kann ein $ack$ eindeutig einem versendeten Datenpaket zugeordnet werden.
...
Fall muss der Subscriber, sollte die Server-Antwort ausfallen, seine Anfrage erneut senden. Dafür sollte er ebenfalls einen Timer mit laufen lassen. Wie bei
TCP sollte sich das $rto$-Intervall stetig vergrößern, um die Serverbelastung nicht noch zu steigern. Erreicht die Server-Antwort den Subscriber
lediglich verspätet, nach dem die Anfrage bereits wiederholt gesendet wurde, kann der Subscriber nicht zuordnen, auf welche Anfrage sich die Antwort bezieht,
da unser Ansatz keine Sequenznummern zulässt. An dieser Stelle muss also eine andere Lösung gefunden werden, um den $rtt$ in zufriedenstellender Weise zu
ermitteln. Alle weiteren Überlegungen und Entscheidungen werden wir im Abschnitt \ref{css:staukontrolle_pubsubrss} erörtern.
