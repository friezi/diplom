...
wird durch $rto$ bestimmt. Das Problem der \glqq retransmission-ambiguity\grqq{} (siehe \ref{csp:karns_algorithmus}) kann nicht eintreten. Soll ein Verbindungsaufbau zu
einem RSS-Server geschehen, so sprechen wir im Folgenden der Einfachheit halber von der Aussendung eines \glqq Feed-Requests\grqq{}.\\
...
Timer $RT$ (für \glqq Retransmission-Timer\grqq{}) eingerichtet werden, nach dessen Ablauf der Feed-Request erneut ausgesandt wird. Die Unterscheidung der beiden Timer
...
Dabei ist $\text{sf}()$ die Skalierungsfunktion und $i$ die Anzahl der ausgesendeten Anfragen bzw. die Anzahl der Verbindungsversuche bis zu einer erfolgreichen Verbindung.
Experimente mit Hilfe der entwickelten Simulationsumgebung haben gezeigt, dass eine exponentielle Skalierungsfunktion ein
...
als proportionalen Anteil in der Berechnung auffassen kann (da er eine genaue Zeitdifferenz ausdrückt), entspricht die Skalierungsfunktion $sf()$ dem integrativen Anteil beim PI-Regler,
da sie die zeitliche Dauer der Server-Antwort berücksichtigt.\\
...
bei einer sehr kleinen Polling-Periode), können die übrigen Subscriber \glqq aussperren\grqq{}. Die Subscriber, welche eine kürzere Polling-Periode berechnet haben, können
...
Subscribern aufgeteilt. Somit erhalten sie die Feeds auch mit einer höheren Frequenz vom Server. Kann der Server der Beantwortung einer Anfrage schnell nachkommen,
so kann der empfangende Subscriber seinen $cpp$ konstant halten. Die Polling-Perioden der
...
Serverbelastung und somit der Polling-Perioden im System führen. Allerdings ist eine schnelle Reaktion des Systems auf eine
