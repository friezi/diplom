...
Im Folgenden werden wir Techniken\todo{welche?}aus diesen Teilgebieten betrachten und diese auf ihre Tauglichkeit bezüglich der Lösung unseres beschriebenen Problems untersuchen.
...
Fall muss der Subscriber, sollte die Server-Antwort ausfallen, seine Anfrage erneut senden. Dafür sollte er ebenfalls einen Timer mitlaufen lassen. Wie bei
...
lediglich verspätet, nachdem die Anfrage bereits wiederholt gesendet wurde, kommt es hier in jedem Fall zum Problem der ``retransmission ambiguity'': der
Subscriber \todo{siehe Korrektur}kann nicht zuordnen, auf welche Anfrage sich die Antwort bezieht,
...
wählen, werden wir in Abschnitt \ref{css:staukontrolle_pubsubrss} erörtern.
...
wobei der $cpp$ hier für den $srtt$ bei TCP steht. Im Unterschied zu TCP reicht bei uns der Wert $cpp$ als Timeout \todo{siehe Korrektur}für die nächste Anfrage jedoch nicht aus,
da es in unserem Fall
...
Timer $RT$ (für ``Retransmission-Timer'') eingerichtet werden, nach dessen Ablauf der Feed-Request erneut ausgesandt wird. Da aber Sequenznummern keine Rolle
...
mitbestimmend für den $RT$ der $rto$. Diese beiden Timer laufen nie parallel. $cpp$ wird ebenfalls je nach Situation
...
Eine untere Schranke für den Wert $cpp$ soll $ppp$ bilden. Da jeder Subscriber $ppp$ individuell festlegen kann, ist es nicht notwendig, $cpp$ kleiner als $ppp$ zu wählen.
...
In diesem Falle kann $rtt$ anscheinend eindeutig bestimmt werden (warum nur anscheinend, werden wir später erörtern). Bestimmend für den $RT$ ist der $rto$,
...
\todo{TCP-SEQUENCENUMMERN}durch welchen Feed-Request der ankommende
Feed ausgelöst wurde. $rtt$ lässt sich in diesem Fall also nicht eindeutig bestimmen. Wie Karn festgestellt hat, ist es sowohl
ungünstig, den zuletzt ausgesandten Feed-Request wie auch den zuerst ausgesandten Feed-Request als Berechnungsgrundlage zu wählen. Es gibt also verschiedene Werte, welche
für die eigentliche Roundtrip-Time in Frage kommen. Wir wollen daher an dieser
...
Hat es $i$ Retransmissionen gegeben (einschließlich erster Transmission), so gilt
$rto=2^i\cdot cpp$. Dann wählen wir
\begin{equation}
  rtt:=\frac{cpp\sum^{i-1}_{k=1}2^kk}{i}+\varDelta t
\end{equation}
Dabei ist $\varDelta t$ die Zeitspanne zwischen der letzten Retransmission und $t_0$, wobei gilt $0\leq\varDelta t\leq 2^i\cdot cpp$. Die Herleitung dieser Formel
...
man auch eine Analogie zum Konzept der PI-Reglern herstellen (obwohl es nur Parallelen in den Grundideen gibt): während man den bereits berechneten $cpp$
